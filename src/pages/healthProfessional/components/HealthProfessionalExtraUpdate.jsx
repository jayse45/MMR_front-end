...
const HealthProfessionalExtraUpdate = () => {
  ...
  const changeLicenseImage = async (evt) => {
    ...
    const dpPresignedUrl = await FetchManager.asyncFetchJSON({
      url: PRESIGNED_URL,
      method: "POST",
      body: {
        fileType: photo.type,
    ...
    if (addedLicense.status === 200) {
      ...
      NotificationManager.notifyUser({
        type: "error",
        message: "Sorry, could not complete the upload process.",
        toastId: 1,
      });
      setLoading(false);
      return false;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      StorageManager.set(
        "licenseVerified",
        res[1]?.body?.licensePicture?.status === "approved"
      );
        ...
        <Grid container spacing={5} justifyContent="center">
          <Grid item xs={12} sm={4} md={4} xl={3} pr={3}>
            <ImageListItem
              key={previewImage}
              sx={{
                backgroundColor: "#26c28129",
                height: "200px!important",
                borderRadius: "3px",
                border: "5px solid #00000080",
              }}
            >
              <img src={previewImage} alt={"license"} loading="lazy" />
              <ImageListItemBar
                sx={{
                          ...
                          >
                            ...
                            >
                              ...
                              <FormControl fullWidth>
                                <FormLabel>
                                  License Image(.jpeg,.jpg,.png, gif)
                                </FormLabel>
                                <TextField
                                  size={"small"}
                                  disabled={loading}
          ...
          <Grid item xs={12} sm={7} md={7} xl={6}>
            <Box component="form" onSubmit={saveBasicBio} sx={{ marginX: 3 }}>
              ...
              <Box my={1}>
                <TextField
                  disabled={["approved", "added"].includes(
                    userData.licensePicture.status
                  )}
                  size="small"
                  fullWidth
                  label="License Pin"
              ...
              <Box mt={1}>
                <Button disabled={loading} onClick={saveBasicBio}>
                  <Save /> Save
                </Button>
              </Box>
            </Box>
