...
const HEALTH_PROFESSIONAL_TYPE_URL = UrlHelper.createApiUrlPath(
  "/api/healthprofessionalTypes"
);
...
export default function OnboardHealthProfessional() {
  ...
  const uploadFile = async (field) => {
    ...
    if (files[field] === "" || files[field] === null) {
      setLoading(false);
      return NotificationManager.notifyUser({
        type: "info",
    ...
    const presignedUrl = await FetchManager.asyncFetchJSON({
      url: PRESIGNED_URL,
      method: "POST",
      body: {
    ...
    if (dpResponse.statusText !== "OK") {
      NotificationManager.notifyUser({
        type: "error",
        message: "Sorry, could not upload image.",
        toastId: 1,
      });
      setLoading(false);
      return false;
    ...
    if (field === "licensePicture") {
      const addedLicense = await FetchManager.asyncFetchJSON({
      ...
      if (addedLicense.status !== 200) {
        NotificationManager.notifyUser({
          type: "error",
          message: "Sorry, could not complete the process.",
          toastId: 1,
        });
        return false;
      }
  ...
  const nextSaveHandler = async (evt) => {
    ...
    switch (step) {
      case 0:
        await saveBasicBio(evt);
        break;
      case 1:
        await uploadFile("photo");
        break;
      case 2:
        await uploadFile("licensePicture");
        completeProfile(evt);
        break;
      default:
        break;
      ...
      <Paper elevation={3}>
        ...
        >
          <img
            ...
            alt="MonitorMyRehab"
          />
          <Typography variant="h5" mt={1} sx={{ fontSize: "1.2rem" }}>
          ...
          <Grid container justifyContent={"center"}>
            {isLoading ? (
              ...
              >
                <Stepper
                  activeStep={step}
                  sx={{ marginY: 1, padding: 1, border: "1px solid #0474e829" }}
                >
                  <Step key={0}>
                    <StepLabel>Basic Bio</StepLabel>
                  </Step>
                  <Step key={2}>
                  ...
                  >
                    <FormControl fullWidth>
                      <TextField
                        size="small"
                        required
                        id="healthProfessionalType"
                        name="healthProfessionalType"
                        label="Area of Specialization"
                        onChange={handleChange}
                        value={userData.healthProfessionalType}
                        select
                      >
                        {healthprofessionaltypes.map((item) => (
                          <MenuItem key={item._id} value={item._id}>
                            {item.name.toLocaleUpperCase()}
                          </MenuItem>
                        ))}
                      </TextField>
                      <FormHelperText id="HealthProfessionalType-helper-text">
                    ...
                    <FormControl fullWidth>
                      ...
                      <FormHelperText id="HealthProfessionalType-helper-text">
                        Year you started your practice
                      </FormHelperText>
                    </FormControl>
                    <Box my={1}>
                      <TextField
                    ...
                    <Box my={1}>
                      <TextField
                        size="small"
                        fullWidth
                        placeholder="Current place of practice"
                        name="workplace"
                        onChange={handleChange}
                ...
                {step === 1 && (
                  <Box sx={{ display: "flex", justifyContent: "center" }}>
                    <Box sx={{ maxWidth: "250px" }}>
                      ...
                      <FormControl fullWidth>
                        <FormLabel>
                          Profile Image(.jpeg,.jpg,.png, gif)
                        </FormLabel>
                        <TextField
                          size={"small"}
                          disabled={isLoading}
                    ...
                    <Box sx={{ maxWidth: "300px" }}>
                      <Box sx={{ display: "flex", justifyContent: "center" }}>
                        <Box
                          ...
                          component="img"
                          src={previewImages.licensePicture}
                          alt="License Picture"
