...
const PATIENTS_URL = UrlHelper.createApiUrlPath("/api/users/users");
const HEALTH_PROFESSIONAL_INFO_URL = UrlHelper.createApiUrlPath(
  "/api/users/healthProfessionalExtra"
);
const APPOINTMENT_URL = UrlHelper.createApiUrlPath("/api/appointments/users");

const now = new Date();
const dateString = `${now.getFullYear()}-${now
  .getMonth()
  .toLocaleString("en", { minimumIntegerDigits: 2 })}-${now
  .getDate()
...
const MakeAppointmentForm = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      FetchManager.asyncFetchJSON({
        url: HEALTH_PROFESSIONAL_INFO_URL,
      }),
    ]).then((res) => {
      setHealthprofessionaltypes(res[0].body);
      setPatients(res[1].body);
      setIsLoading(false);
    });
  }, []);
  return (
        ...
        <Box component={"form"}>
          ...
          <Box sx={{ marginTop: 2 }}>
            <FormControl fullWidth>
              ...
              <FormHelperText id="patient-helper-text">
                Patient you are creating appointment for.
              </FormHelperText>
            </FormControl>
          </Box>
          <Box sx={{ marginTop: 2 }}>
            <FormControl fullWidth>
              <TextField
                ...
                onChange={handleFormChange}
                value={formData.healthProfessionalType}
                select
              >
              ...
              <FormHelperText id="HealthProfessionalType-helper-text">
                Choose the type of Health Professional you want to meet.
              </FormHelperText>
            </FormControl>
          </Box>
          <Box>
          ...
          <Box>
            <FormControl>
              ...
              <FormHelperText id="my-helper-text">
                Select data and time for appointment.
              </FormHelperText>
            </FormControl>
          </Box>
          <Box>
