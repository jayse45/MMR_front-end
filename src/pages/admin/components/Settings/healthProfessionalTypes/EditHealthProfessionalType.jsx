...
const EditHealthProfessionalType = ({HealthProfessionalType}) => {
	const [loading, setLoading] = useState(false);
	const [formData, setFormData] = useState({
		name: HealthProfessionalType.name,
	...
	const handleSubmission = (evt) => {
		...
		FetchManager.fetch({
			...
			success_cb: (res) => {
				if (res.status === 200) {
					NotificationManager.notifyUser({ message: "Health Professional Type updated successfully", type: "success", toastId: 1 })
				} else {
			...
			failure_cb: (res) => {
				NotificationManager.notifyUser({ message: "Failed to update Health Professional Type", type: "warning", toastId: 1 })
				setLoading(false);
			}
		})
	}
	...
	return (
		<Box sx={{ minWidth: "500px" }}>
			...
			{!loading &&
				<Box component="form">
					<Typography variant='subtile2' >Update: <i>{HealthProfessionalType.name}</i></Typography>
					<Box sx={{ marginTop: 2 }}>
...
export default EditHealthProfessionalType;
