...
import { TextField, FormControl, Box, Typography, FormLabel, CircularProgress, Stepper, StepLabel, Step, MenuItem } from '@mui/material';
...
const AddExerciseForm = ({ success_cb }) => {
	...
	const handleTextSubmission = async (evt) => {
		...
		const res = await FetchManager.asycnFetchJSON({
			...
			failure_cb: () => {
				setIsLoading(false);
				NotificationManager.notifyUser({ type: "warning", message: "Failed to add exercise." })
			}
		});
		if (res?.status === 201) {
			NotificationManager.notifyUser({ type: "success", message: "Exercise created. Upload files." })
	...
	return (
		<Box sx={{ minWidth: "600px", maxHeight: "400px" }}>
			<Typography mb={2} variant='h5' >Add Exercise</Typography>
				...
				<Box p={3} sx={{minWidth: "500px" }}>
					...
					{step === 4 &&
						<Box my={3} sx={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
							...
							<Typography variant="subtitle2">Exercise Added Successfully</Typography>
						</Box>
					}
				</Box>
			</Fragment>
