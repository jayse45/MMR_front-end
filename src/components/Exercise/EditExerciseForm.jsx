...
const EditExerciseForm = ({ success_cb, exercise }) => {
  ...
  const [placeholders, setPlaceholders] = useState({
    thumbImage:
      currentExercise?.thumbImage?.url ?? "/static/img/placeholder-img.jpg",
  ...
  const handleTextSubmission = async (evt) => {
    ...
    const res = await FetchManager.asyncFetch({
      method: "PUT",
  ...
  return (
    <Box sx={{ minWidth: "600px", maxHeight: "400px" }}>
      ...
      <Box mt={2} px={3} display={"flex"} justifyContent={"space-between"}>
        ...
        <Button type="submit" disabled={isLoading} onClick={nextSaveHandler}>
          Save
        </Button>
        <Button
          type="submit"
