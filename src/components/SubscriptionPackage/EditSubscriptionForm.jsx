...
const EditSubscriptionPackageForm = ({ subscriptionPackage }) => {
	...
	const [formData, setFormData] = useState({
		...
		duration_text: subscriptionPackage.duration_text,
		disabled: subscriptionPackage.disabled,
	...
	const [descriptionText, setDescriptionText] = useState(RichTextEditor.createValueFromString(subscriptionPackage.description, 'html'));
	...
	const handleSubmission = (evt) => {
		...
		FetchManager.fetch({
			url: SUBSCRIPTION_URL + subscriptionPackage._id,
			method: "PUT",
			body,
			success_cb: (res) => {
				if (res.status === 200) {
					NotificationManager.notifyUser({ message: "Package updated successfully", type: "success", toastId: 1 })
					setTimeout(() => {
			...
			failure_cb: (res) => {
				NotificationManager.notifyUser({ message: "Failed to updated package", type: "warning", toastId: 1 })
				setLoading(false);
			}
		})
	}
	...
	return (
		<Box sx={{ minWidth: "500px" }}>
			...
			{!loading &&
				<Box component="form">
					<Typography variant='h5' >Edit Subscription Package</Typography>
					...
					<Box>
						<Button type="submit" onClick={handleSubmission}>Edit Package</Button>
					</Box>
				</Box>
			}
		</Box>
...
export default EditSubscriptionPackageForm;
